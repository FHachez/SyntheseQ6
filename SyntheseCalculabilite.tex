...
\begin{itemize}
	...
	\item un langage est un ensemble de mots constitués de symboles d'un alphabet
		donné.
	\item ensemble de tous les mots possibles sur l'alphabet $\sum$ : $\sum ^*$
\end{itemize}
...
\begin{mytheo}[S-m-n]
	On va montrer comment construire un programme qui calcule 
	$S^n(k,x_{n+1}, ...,x_{n+m})$.\\
	Tout d'abord, comme $\phi_k$ est calculable, il existe : $P_k(x_1,x_2,...,x_{n+m})$.\\
	On peut donc construire un programme $Q(x_1,...,x_n)$ qui calcule 
	$P_k(x_1,x_2,...,x_{n+m})$.\\
	Notre programme qui calcule $S^n(k,x_{n+1}, ...,x_{n+m})$ n'a plus 
	qu'a retourner le numéro du programme Q.\\
       	En résumé, $S^n(k,x_{n+1},
	...,x_{n+m})$ = 
	\begin{lstlisting}
construire Q(x1,x2,...,xn) = P(x1,x2,...xn,...,xn+m)
print le numero de Q
	\end{lstlisting}
\end{mytheo}
