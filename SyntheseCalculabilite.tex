...
\begin{mytheo}
	\label{S-m-n}[S-m-n :] Pour tout $m,n \leq 0$, \\
...
\begin{mytheo}
	\label{point-fixe}[Point fixe :] Soient $n \geq 0$ et f : fonction totale 
...
Lors de l'étude de la complexité, on ne va considérer que la borne supérieur 
...
\begin{myrem}
	...
	donnée (simplex). Et celles-ci sont souvent des cas particuliers et peux 
...
\begin{myrem}
	Ce qui signifie que ce problèmes appartient à la classe de problème et 
	est A-difficile.	
\end{myrem}
...
\begin{mydef}[Réduction fonctionnelle]
	...
	($A\leq_f B$) si il existe une fonction \textbf{total calculable} f 
	telle que 
	\[ a\in A \ \iff \ f(a) \in B \]
\end{mydef}
...
obliger d'utiliser un certain schéma d'algorithme :
...
On est donc limiter à utiliser qu'une fois le test $f(a) \in B$ \textbf{en 
	dernier lieux}.

\subsection{Modèles de calcul}
D'habitude, on utilise les machines de Turing pour avoir une définition précise 
de la complexité. Mais c'est peu intuitif et on s'intéresse au frontière. Or la 
différence de complexité entre différent modèle est un facteur polynomial 
