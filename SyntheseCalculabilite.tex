...
\paragraph{} Mais il existe aussi des langages qui ne sont pas complet comme le 
...
\begin{description}
	\item[Ruban] Suite de case potentiellement infinie (des 2 cotés) mais à 
...
\label{ssub:contr_le}
Le contrôleur est composé d'un nombre d'état fini dont un état initial et un 
...
Au départ il y a juste les données sur le ruban. Sur les autres case il y a le 
symbole B. La tête de lecture se trouve sur la première case. Tant que c'est 
possible on applique des instructions. Il y a 2 cas possible pour l'arrêt soit 
...
\begin{mydef}
	\textbf{T-calculable} Une fonction f est T-calculable si il existe une machine de Turing qui, 
	recevant comme donnée n'importe quel nombre entier x fourni tôt ou tard 
	comme résultat f(x) si celui-ci existe.
\end{mydef}

\begin{mydef}
	\textbf{T-récursif} Soit $A\subseteq \N$, A est T-récursif si il existe 
...
\begin{mydef}
	...
	T-récursivement énumérable si il existe 
	une machine de Turing qui, recevant comme donnée n'importe quel nombre 
	naturel x, fourni tôt ou tard comme résultat : \\
	\begin{tabular}{l}
		1 si $x\in A$ \\
