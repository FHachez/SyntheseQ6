...
La synthèse est assez longue mais elle reprend ce qui est selon moi intéressant à connaitre
...
\begin{enumerate}
	...
	\item Sélectionner la diagonale
...
\end{enumerate}
On montre que lorsqu'on essaye d'énumérer on loupe toujours des éléments.
...
\begin{enumerate}
	\item On peut donc construire une table infinie : \\
		\begin{tabular}{|c||c|c|c|c|c|c|}
			...
			$x_1$ & $x_{11}$ & $x_{11}$ & $x_{12}$ & ... & $x_{1k}$ & ... \\
			$x_2$ & $x_{22}$ & $x_{21}$ & $x_{22}$ & ... & $x_{2k}$ & ... \\
			: & : &:& : & : & : &:\\
			$x_k$ & $x_{kk}$ & $x_{k1}$ & $x_{k2}$ & ... & $x_{kk}$ & ... \\
			: & : &:& : & : & : &:\\
			\hline
		\end{tabular}
	...
	\item Contradiction car $d'$ est dans l'énumération car E est
		...
		La contradiction vient du fait qu'on a choisis $x_{pp}' \neq
...
\begin{myrem}
	...
	vert sur Mars ou encore x occurrences de 5 dans $\pi$.
\end{myrem}
...
d'autre problème calculable. Il y a d'autres exemples sur les grammaires dans le cours.
...
\begin{itemize}
	...
	terminal. 
	\item A et B sont des symboles non terminal
\end{itemize}
...
\paragraph{Plusieurs rubans}C'est-à-dire la tête peut se déplacer dans des directions 
...
Une telle machine est possible à construire. Il y a plusieurs façons de faire
différents. Une façon de faire est d'utiliser 3 rubans:
\begin{itemize}
...
\begin{myrem}
	Les propriétés sur les descriptions (SD et CD) peuvent être vues comme
	des il existe. On dit qu'il existe une description.
	Alors que celles sur les algorithmes (SA, CA) sont plus concrètes en disant qu'on 
